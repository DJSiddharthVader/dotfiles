#!/bin/bash
set -o pipefail # no -e causes gtkautoreload to fail since it relies on timeout
...
wall() {
    mode="$1"
    [[ -d "$mode" ]] && mode="$(find "$mode" -maxdepth 99 -type f | shuf | head -1)"
    # get index of new wallpaper
    index=$(update_index "$mode") 
    # write index to file
    set_index "$index" 
...
display() {
    # replace WALLPAPER_DIR with ... in wallpaper path
    wallpaper="$(get_image_index "$(get_index)" | sed -E 's/^.*wallpapers\/(.*)$/...\/\1/')"  
    echo "$wallpaper"
